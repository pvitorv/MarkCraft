...
export class ImageStudioEngine {
    ...
    async addSvgIconFromSpec(spec) {
        ...
        const color = resolveInsertFill(spec.fill, DEFAULT_SHAPE_FILL);
        const applyFill = (obj) => {
            if (!obj) return;
    ...
    async addElementFromCatalog(spec) {
        ...
        if (type === 'emoji' || type === 'sticker') {
            ...
            this.addText(char, {
                ...
                fill: resolveInsertFill(spec.fill, DEFAULT_SHAPE_FILL),
    ...
    return {
        ...
        filterImageStudioElements() {
            ...
            return list.filter((el) => {
                ...
                if (groupFilter && !this.imageStudioElementMatchesNav(el, groupFilter)) {
                    return false;
                }
                if (q) {
        ...
        imageStudioElementMatchesNav(el, navId) {
            ...
            if (navId === 'formas') {
                return group.startsWith('formas') || group === 'linhas';
            }
            if (navId === 'icones') {
                return type === 'icon_glyph' || type === 'svg_icon' || group === 'icones' || group.startsWith('bs_');
            }
            if (navId === 'icones_fa') {
                return type === 'icon_glyph' && String(el.font || '').startsWith('fa_');
            }
            if (navId === 'icones_material') {
                return type === 'icon_glyph' && el.font === 'material_symbols';
            }
            if (navId === 'icones_bootstrap') {
                return type === 'svg_icon' || group.startsWith('bs_');
            }
            if (navId.startsWith('formas_') || navId === 'linhas' || navId === 'emojis' || navId === 'blobs' || navId === 'adesivos') {
                return group === navId;
            }
            if (navId.startsWith('bs_')) {
                return group === navId;
            }
            ...
            return [
                {
                    title: 'Biblioteca',
                    items: [
                        { id: '', label: 'Todos os elementos', count: all.length },
                    ],
                },
                {
                    title: 'Formas',
        ...
        imageStudioElementQuickGroups() {
            return this.imageStudioElementNavSections().flatMap((s) => s.items);
        },
        ...
        async imageStudioAddElementFromModal(el) {
            ...
            // Mantém o modal aberto para adicionar vários; feedback na barra de mensagem.
        },
        ...
        async loadImageStudioDesign() {
            ...
            try {
                const { data } = await api.get('/image-studio/workspace', {
                    params: { preset: this.imageStudioPreset || 'custom' },
                });
        ...
        imageStudioOnShapeStrokeWidthChange() {
            if (this.imageStudioShapeStrokeWidth > 0 && !this.imageStudioShapeStroke) {
                this.imageStudioShapeStroke = DEFAULT_SHAPE_STROKE;
            }
            this.imageStudioOnShapeFillChange();
        },
        ...
        scheduleImageStudioSave() {
            // MarkCraft: artes não são persistidas no servidor — só memória da aba.
        ...
        async clearImageStudioWorkspace(force = false) {
            if (!force) {
                const ok = confirm(
                    'Limpar o workspace descarta o trabalho atual. Baixe o export antes, se quiser manter. Continuar?'
                );
                if (!ok) {
                    return;
                }
        ...
        async imageStudioAddIconGlyph(glyph) {
            ...
            this.imageStudioEngine.addText(glyph.char, {
                ...
                fill: resolveInsertFill(this.imageStudioTextFill, DEFAULT_SHAPE_FILL),
                stroke: this.imageStudioTextStroke,
                strokeWidth: this.imageStudioTextStrokeWidth,
                textAlign: 'center',
                fontSlug: glyph.font,
            ...
            // Ícones em fonte costumam herdar fill de texto branco — força destaque no insert.
            const active = this.imageStudioEngine.getActiveObject();
            if (active && isFabricText(active)) {
                active.set('fill', resolveInsertFill(active.fill, DEFAULT_SHAPE_FILL));
        ...
        imageStudioResolveThumbnailPlatform() {
            return this.imageStudioPresetPlatformMap?.[this.imageStudioPreset] || null;
        },

        async imageStudioPushThumbnail() {
            this.message = 'No MarkCraft a arte é baixada para o seu PC — não há módulo Thumbnail.';
        },

        async imageStudioPushLibrary() {
            this.message = 'No MarkCraft não há biblioteca no servidor. Use Baixar e importe de novo quando precisar.';
        },
