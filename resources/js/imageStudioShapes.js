...
/** Cor inicial das formas — contraste com prancheta branca (editável depois). */
export const DEFAULT_SHAPE_FILL = '#0d9488';
export const DEFAULT_SHAPE_STROKE = '#0f766e';
...
/** No insert: branco/ausente → cor de destaque; outras cores do catálogo são respeitadas. */
export function resolveInsertFill(color, fallback = DEFAULT_SHAPE_FILL) {
    if (color == null || color === '' || isNearWhiteColor(color)) {
        return fallback;
    }
    return color;
}

export function resolveInsertStroke(color, fallback = DEFAULT_SHAPE_STROKE) {
    if (color == null || color === '' || isNearWhiteColor(color)) {
        return fallback;
    }
    return color;
...
function solidPaint(spec, fallbackFill = DEFAULT_SHAPE_FILL) {
    return {
        fill: resolveInsertFill(spec.fill, fallbackFill),
        stroke: spec.stroke ? resolveInsertStroke(spec.stroke, DEFAULT_SHAPE_STROKE) : '',
        strokeWidth: spec.strokeWidth || 0,
    };
}

function outlinePaint(spec, fallbackStroke = DEFAULT_SHAPE_STROKE, fallbackWidth = 8) {
    return {
        fill: 'transparent',
        stroke: resolveInsertStroke(spec.stroke ?? spec.fill, fallbackStroke),
...
export function createShapeFromSpec(spec, designWidth, designHeight) {
    ...
    if (type === 'frame_rect' || type === 'frame_rounded') {
        ...
        return new Rect({
            ...
            rx,
            ry: spec.ry ?? rx,
            ...outlinePaint(spec, DEFAULT_SHAPE_STROKE, spec.strokeWidth || 24),
            ...meta,
        });
    }
    ...
    if (type === 'ring') {
        return new Circle({
            radius: size / 2,
            fill: 'transparent',
            stroke: resolveInsertStroke(spec.stroke ?? spec.fill, DEFAULT_SHAPE_STROKE),
    ...
    if (type === 'star_outline') {
        const points = starPolygonPoints(size / 2, size / 4, spec.spikes || 5);
        return polygonFromPoints(points, spec, id, baseName, opacity, outlinePaint(spec, DEFAULT_SHAPE_STROKE, 6));
    }
    ...
    const pathTypes = {
        ...
        arrow_outline: { key: 'arrow_outline', paint: outlinePaint(spec, DEFAULT_SHAPE_STROKE, 4) },
        ...
        arc: { key: 'arc', paint: { fill: '', stroke: resolveInsertStroke(spec.stroke ?? spec.fill, DEFAULT_SHAPE_STROKE), strokeWidth: spec.strokeWidth || 10, strokeLineCap: 'round' } },
        cross: { key: 'cross', paint: solidPaint(spec) },
        cube: { key: 'cube', paint: solidPaint(spec) },
        cube_outline: { key: 'cube_outline', paint: outlinePaint(spec, DEFAULT_SHAPE_STROKE, 4) },
        ...
        corner_bracket: { key: 'corner_bracket', paint: outlinePaint(spec, DEFAULT_SHAPE_STROKE, 10) },
    };
