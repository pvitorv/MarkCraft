@php
    ...
    $inspireImg = \App\Support\Cms::normalizeStoragePath((string) ($inspire['image'] ?? ''));
    $designImg = \App\Support\Cms::normalizeStoragePath((string) ($design['image'] ?? ''));
@endphp

<section class="mx-auto max-w-6xl px-4 pt-4 pb-6 sm:pt-6 sm:pb-8" aria-label="Inspiração e dicas">
    <div class="grid gap-4 md:grid-cols-2">
        <article class="mc-showcase-card overflow-hidden rounded-xl border border-white/10 bg-zinc-950/40">
            <div class="mc-showcase-art relative min-h-[10.5rem] border-b border-white/5">
                @if($inspireImg !== '')
                    <img src="{{ $inspireImg }}" alt="" class="absolute inset-0 h-full w-full object-cover">
                @else
        ...
        <article class="mc-showcase-card overflow-hidden rounded-xl border border-white/10 bg-zinc-950/40">
            <div class="mc-showcase-art relative min-h-[10.5rem] border-b border-white/5">
                @if($designImg !== '')
                    <img src="{{ $designImg }}" alt="" class="absolute inset-0 h-full w-full object-cover">
                @else
            ...
            <div class="px-4 py-4 sm:px-5">
                ...
                <h2 class="mc-brand mt-1 text-lg font-bold text-white">{{ $design['title'] ?? '' }}</h2>
                <p class="mt-2 text-sm text-zinc-400 leading-relaxed">{{ $design['text'] ?? '' }}</p>
            </div>
        </article>
    </div>
</section>
