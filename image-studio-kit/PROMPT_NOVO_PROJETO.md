...
> **Instrução ao Agent:** leia primeiro `HISTORICO_ATUALIZACOES.md` e `GUIA_REMBG_SUBSTITUIR_IMGLY.md`, depois este arquivo, `DEPENDENCIAS.md`, `INVENTARIO.md` e o código em `source/`.  
> Remoção de fundo: padrão **rembg no servidor** (não IMG.LY). Implemente o produto **MarkCraft** — app web **novo**, não continue o CriaSys Editor de slideshow.
...
5. Remover fundo com **rembg no servidor** (`BackgroundRemovalService` + `scripts/remove-background.py`). Driver `IMAGE_STUDIO_BG_REMOVAL_DRIVER=rembg`. IMG.LY (`imgly`) só se o dono pedir e tiver licença; ver `GUIA_REMBG_SUBSTITUIR_IMGLY.md`.
...
- Espelho atualizado a partir do **BlogCriaSysWeb** branch `014` (código Studio da `013`).
- Histórico antigo do Editor: handoffs `017`–`027` em `source/handoffs/`.
- Apagar a pasta `image-studio-kit` na origem **não quebra** o blog.
- Documentos de verdade para migração técnica: `HISTORICO_ATUALIZACOES.md` + `GUIA_REMBG_SUBSTITUIR_IMGLY.md`.

---

## 13. Primeira mensagem sugerida ao Agent (novo repo / segundo projeto)

```
Leia image-studio-kit/HISTORICO_ATUALIZACOES.md, GUIA_REMBG_SUBSTITUIR_IMGLY.md,
PROMPT_NOVO_PROJETO.md, DEPENDENCIAS.md e INVENTARIO.md.
Atualize/integre o Image Studio com o código em source/ (paths BlogCriaSysWeb).
Remoção de fundo: rembg no servidor (não IMG.LY por padrão) — siga o GUIA_REMBG.
Layouts ≠ Pacotes (nunca union no modal Layouts).
Se for MarkCraft: APP_NAME=MarkCraft, auth para /studio, Baixar + Limpar sem persistir artes,
hub /ferramentas stubs, sem vídeo/TTS.
```

---

*Atualizado em 2026-07-28 — kit sincronizado com BlogCriaSysWeb (rembg, Pacotes, Layouts).*
*Origem anterior: CriaSys Editor; origem atual do espelho: BlogCriaSysWeb.*
